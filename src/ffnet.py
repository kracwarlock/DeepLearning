...
class FFnet:
	...
	def __init__(self, epsilon, momentum, num_epochs, batch_size, num_batches, dim_in, dim_out, num_hid):
		...
		self.wu_w1 = cm.empty(w_w1.shape).assign(0)
		self.wu_b1 = cm.empty(w_b1.shape).assign(0)
		self.wu_w2 = cm.empty(w_w2.shape).assign(0)
		self.wu_b2 = cm.empty(w_b2.shape).assign(0)

		# initialize temporary storage
		self.h = cm.empty((num_hid, batch_size))
		self.out = cm.empty((dim_out, batch_size))
		self.delta = cm.empty((num_hid, batch_size))

	def reinitTestStorage(self, kkk):
		# Initalize temporary storage.
		self.h = cm.empty((num_hid, kkk))
		self.out = cm.empty((dim_out, kkk))
	...
	def train(self, dev_train, dev_lbl):
		...
		for epoch in range(num_epochs):
		    ...
		    for batch in range(num_batches):
		        ...
		        inp = self.dev_train.slice(batch*self.batch_size,(batch + 1)*self.batch_size)
		        target = self.dev_lbl.slice(batch*self.batch_size,(batch + 1)*self.batch_size)
		        ...
		        cm.dot(self.w_w2.T, h, target = self.out)
		        ...
		        self.wu_w1.add_dot(self.inp, self.delta.T, beta = self.momentum)
